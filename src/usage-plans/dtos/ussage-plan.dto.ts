...
export class UsagePlanDto {
  ...
  @ApiProperty({ description: 'The description of the usage plan (If there is one)' })
  description?: string;

  @ApiProperty({ description: 'The Stripe product ID associated with the usage plan. Free plan does not have a product id' })
  stripeProductId: string;

  @ApiProperty({ description: 'The monthly price of the usage plan' })
  priceMonthly: number;

  @ApiProperty({ description: 'The monthly price of the usage plan if pre-paid on a yearly basis' })
  priceYearly: number;
